...
shiny::observeEvent(input$submit, {
  ...
  name_valid <- nf_validate(input$name, rules = list(required = TRUE),
                            question_label = "Name")
  age_valid <- nf_validate(input$age,
                           rules = list(required = TRUE, type = "numeric"),
                           question_label = "Age")

  if (!name_valid$valid) {
    shiny::showNotification(name_valid$errors[[1]], type = "error")
    return()
  }

  if (!age_valid$valid) {
    shiny::showNotification(age_valid$errors[[1]], type = "error")
    return()
  }
